...
class DataGridHandler extends AbstractDataGridHandler
{
    ...
    public function buildQuery()
    {
        ...
        // Getting count by rebuilding the query and removing any 'orderBy' parts
        $queryParts = $queryBuilderEvent->getQueryBuilder()->getDQLParts();

        //remove groupBy
        $queryParts['orderBy'] = array();

        $qb2 = clone $this->dataGrid->getQueryBuilder();
        $qb2->resetDQLParts();

        foreach ($queryParts as $part => $elements) {
            if (is_array($queryParts[$part])) {
                foreach ($queryParts[$part] as $idx => $element) {
                    if (is_object($element)) {
                        $qb2->add($part, $element, true);
                    } elseif (is_array($element)) {
                        foreach ($element as $key => $value) {
                            if (is_object($value)) {
                                $qb2->add($part, array($key => $value), true);
                            }
                        }
                    }
                }
            } elseif (is_object($elements)) {
                $qb2->add($part, $elements);
            }
        }

        $paginator = new Paginator($qb2->getQuery());
